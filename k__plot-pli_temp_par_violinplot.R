...
outdir <- 'violinplot_pli_nigeria__temp_par'
...
# save
ggsave( file = paste0( outdir, '/pli-parietal-temporal-violin.png' ), plot = p, dpi = 200, height = 8, width = 12 )

# write raw data for stats
write.csv( fc, file = paste0( outdir, '/pli-parietal-temporal-violin.csv' ) )
...
p.delta <- ggplot( data = s.m.avg, aes( x = Eyes, y = mean, color = group, fill = group ) ) + 
            ...
            theme( legend.position = 'top', axis.title = element_text( face = "bold" ) ) 
# save
ggsave( file = paste0( outdir, '/pli-temporal-parietal__delta_violin.png' ), plot = p.delta, dpi = 200, height = 8, width = 5)
