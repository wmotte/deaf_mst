...
get.functional.connectivity.data <- function( indir, metadir )
{
	...
	# only keep Occipital and Parietal channel data
...
# FC between occipital - parietal cortex
p <- ggplot( data = s, aes( x = Eyes, y = mean, ymin = mean - ci.dev, ymax = mean + ci.dev, color = group, fill = group ) ) + 
...
# save
ggsave( file = paste0( outdir, '/pli-occipital-parietal.png' ), plot = p, dpi = 200, height = 8, width = 12 )

# write raw data for stats
write.csv( fc, file = paste0( outdir, '/pli-occipital-parietal.csv' ) )
...
# write output
write.csv( res, file = paste0( outdir, '/group_differences_in_occ-par_pli-FC__bayes-factors.csv' ) )
...
grouped <- dplyr::group_by( s.m, group, bands, meta.channel, meta.variable )
...
# write to file
write.csv( s.m.avg, file = paste0( outdir, '/delta_FC_values_used_for_delta_plot.csv' ) )

# FC between occipital - parietal cortex
p.delta <- ggplot( data = s.m.avg, aes( x = Eyes, y = mean, ymin = mean - ci.dev, ymax = mean + ci.dev, color = group, fill = group ) ) + 
			facet_wrap( ~bands, nrow = 1 ) + #, scale = 'free_y' ) + 
			geom_bar( stat = 'identity', position = position_dodge(), alpha = 0.8, color = 'gray30' ) + 
			geom_errorbar( stat = 'identity', position = position_dodge( 0.9), color = 'gray70', width = 0.4 ) + 
            ...
            theme( legend.position = 'top', axis.title = element_text( face = "bold" ) ) 
# save
ggsave( file = paste0( outdir, '/pli-occipital-parietal__delta.png' ), plot = p.delta, dpi = 200, height = 8, width = 5)
