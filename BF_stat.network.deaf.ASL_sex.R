...
get.bf <- function( data, freq, metric )
{
	...
	# bayes
	withAsl <- lmBF( y ~ asl_duration + Eyes + sex, data = data )
	noAsl <- lmBF( y ~ Eyes + sex, data = data )
...
outdir <- 'stat_ASL_deaf_correlations_nigeria_sex'
...
# write output
write.csv( res, file = paste0( outdir, '/deaf__bayes_factors_asl_sex.csv' ) )
