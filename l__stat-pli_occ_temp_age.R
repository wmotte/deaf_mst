...
get.bf <- function( data, freq )
{
	...
	# bayes
	withGroup <- lmBF( value ~ group + Eyes + age, data = data )
	noGroup <- lmBF( value ~ Eyes + age, data = data )
...
outdir <- 'stats_pli_nigeria__occ_temp_age'
...
write.csv( fc, file = paste0( outdir, '/pli-temporal-occipital.csv' ) )
...
# write output
write.csv( res, file = paste0( outdir, '/group_differences_in_occ-temp_pli-FC_age__bayes-factors.csv' ) )
