...
get.bf <- function( data, freq, metric )
{
	...
	# bayes
	withAsl <- lmBF( y ~ asl_duration + Eyes, data = data )
	noAsl <- lmBF( y ~ Eyes, data = data )
...
for( freq in freqs )
{
	for( metric in metrics )
	{
		...
		all <- rbind( all, get.bf( df, freq, metric ) )
	}
}
...
# write output
write.csv( res, file = paste0( outdir, '/deaf__bayes_factors_asl.csv' ) )
