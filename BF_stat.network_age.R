...
get.bf <- function( data, freq, metric )
{
	...
	# bayes
	withInteraction <- lmBF( y ~ group + Eyes + age + group:Eyes, data = data )
	noInteraction <- lmBF( y ~ group + Eyes + age, data = data )
...
outdir <- 'stat_network_nigeria_age'
...
# write output
write.csv( res, file = paste0( outdir, '/bayes_factors_group-condition-interaction_age.csv' ) )
...
# write output
write.csv( res, file = paste0( outdir, '/bayes_factors_deafness-condition-interaction_age.csv' ) )
