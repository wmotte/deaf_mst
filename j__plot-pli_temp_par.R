...
outdir <- 'plot_pli_nigeria__temp_par'
...
# save
ggsave( file = paste0( outdir, '/pli-parietal-temporal.png' ), plot = p, dpi = 200, height = 8, width = 12 )
...
# write output
write.csv( res, file = paste0( outdir, '/group_differences_in_temp-par_pli-FC__bayes-factors.csv' ) )
...
p.delta <- ggplot( data = s.m.avg, aes( x = Eyes, y = mean, ymin = mean - ci.dev, ymax = mean + ci.dev, color = group, fill = group ) ) + 
            ...
            theme( legend.position = 'top', axis.title = element_text( face = "bold" ) ) 
# save
ggsave( file = paste0( outdir, '/pli-temporal-parietal__delta.png' ), plot = p.delta, dpi = 200, height = 8, width = 5)
