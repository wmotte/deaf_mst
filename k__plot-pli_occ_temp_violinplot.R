...
get.functional.connectivity.data <- function( indir, metadir )
{
	...
	# only keep Occipital and Temporal channel data
	head( df.sub <- df[ df$meta.channel == 'Occipital' | df$meta.channel == 'Temporal', ] )
...
outdir <- 'violinplot_pli_nigeria__occ_temp'
...
# save
ggsave( file = paste0( outdir, '/pli-temporal-occipital-violin.png' ), plot = p, dpi = 200, height = 8, width = 12 )

# write raw data for stats
write.csv( fc, file = paste0( outdir, '/pli-temporal-occipital-violin.csv' ) )
...
p.delta <- ggplot( data = s.m.avg, aes( x = Eyes, y = mean, color = group, fill = group ) ) + 
            ...
            theme( legend.position = 'top', axis.title = element_text( face = "bold" ) ) 
# save
ggsave( file = paste0( outdir, '/pli-occipital-temporal__delta_violin.png' ), plot = p.delta, dpi = 200, height = 8, width = 5)
